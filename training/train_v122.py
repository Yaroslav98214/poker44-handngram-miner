"""
Train v122: AP-first calibration for R5 mixed/high batches.

Fixes v120 R5 failure: live-compressed score_remap (threshold~0.10) remaps
high-raw live batches (0.58-0.64) into a flat ~0.62 band, predicting 100/100
bots and triggering FPR cliff on mixed batches (simulated FPR=56% -> reward=0).
"""
import hashlib
import sys
import time
...
print("=== v122 Hybrid Training (AP-first + multi-scenario calibration) ===", flush=True)
t0 = time.time()
...
HOLDOUT_DATES = {"2026-06-30", "2026-07-01"}
RECENT_DATES = {
...
BATCH_HUMANS = 50
QUANTILE_BLEND = 0.55
LOW_RAW_LO, LOW_RAW_HI = 0.078, 0.144
HIGH_RAW_LO, HIGH_RAW_HI = 0.586, 0.641
...
for bias in np.arange(-0.5, 2.5, 0.05):
    ...
    for name, raw, labels in scenarios:
        ...
        if fpr >= MAX_FPR:
            ...
            break
        rewards.append(rew)
        if name in ("holdout", "mixed50", "live_fixed"):
...
meta_info = {
    ...
    "model_name": "poker44-v122-hybrid",
    "model_version": "1.22.0",
    "framework": "hybrid-lgb-xgb-et-hgram-quantile-apfirst",
    ...
    "calibration_notes": f"AP-first remap, quantile_blend={QUANTILE_BLEND}, multi-scenario bias",
...
out_path = "models/poker44_v122_deploy.joblib"
