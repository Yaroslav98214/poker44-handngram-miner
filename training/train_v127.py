"""Train v127: Jul 9 benchmark + live-batch bot-rate calibration guard."""
import ast
import hashlib
import re
import sys
...
print("=== v127 Hybrid Training (Jul 9 benchmark + live bot-rate guard) ===", flush=True)
t0 = time.time()
...
HOLDOUT_DATES = {"2026-07-08", "2026-07-09"}
RECENT_DATES = {
...
RECENCY_BOOST = 12.0
MAX_FPR = 0.10
MAX_LIVE_BOT_RATE = 0.58
TARGET_LIVE_BOT_RATE = 0.50
MIN_TOKEN = 40
QUANTILE_BLEND = 0.0
DEFAULT_CAL = {"lo": 0.18, "span": 0.85, "bias": 0.50}
...
def eval_cal(raw_test, y_test, arena_raw, arena_labels, live_raw, lo, span, bias):
    ...
    live_final = pipeline_scores(live_raw, lo, span, bias) if live_raw is not None else None
    live_bot_rate = float((live_final >= 0.5).mean()) if live_final is not None else 0.5
    ...
    return {
        ...
        "bot_recall": bot_recall, "live_bot_rate": live_bot_rate,
        "score": (hold_rew * 0.40 + arena_rew * 0.35 + live_penalty * 0.25),
    }
...
def main() -> None:
    ...
    print("Calibrating batch_rank with live bot-rate guard...", flush=True)
    best = None
    for lo in np.arange(0.08, 0.22, 0.02):
        for span in np.arange(0.50, 0.92, 0.04):
            for bias in np.arange(0.30, 0.80, 0.05):
                m = eval_cal(raw_test, y_test, arena_raw, arena_labels, live_arena_raw, lo, span, bias)
                if m["hold_fpr"] >= MAX_FPR or m["hold_recall"] < 0.80:
                    continue
                if m["arena_fpr"] >= MAX_FPR or m["arena_recall"] < 0.72:
                    continue
                if m["bot_recall"] < 0.82:
                    continue
                if live_arena_raw is not None and (
                    m["live_bot_rate"] > MAX_LIVE_BOT_RATE or m["live_bot_rate"] < 0.42
                ):
                    continue
    ...
    if best is None:
        ...
        best = eval_cal(
            raw_test, y_test, arena_raw, arena_labels, live_arena_raw,
    ...
    print(
        ...
        f"live_bot_rate={best['live_bot_rate']:.1%}",
        flush=True,
    )
    ...
    meta_info = {
        ...
        "model_name": "poker44-v127-hybrid",
        "model_version": "1.27.0",
        "framework": "hybrid-lgb-xgb-et-hgram-v127-jul9",
        ...
        "calibration_notes": (
            f"v127 Jul9 benchmark; batch_rank lo={best['lo']:.2f} span={best['span']:.2f} "
            f"bias={best['bias']:.2f}; live bot-rate guard"
        ),
        ...
        "bias_source": "jul9_holdout_arena_live_grid",
    }
    ...
    out_path = "models/poker44_v127_deploy.joblib"
